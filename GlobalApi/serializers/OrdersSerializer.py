...
class ListOrderSerializer(serializer.ModelSerializer):
    
    order_client_id =  ClientInformationSerializer(many=False)
    order_product_units = ProductUnitsSerializer(many = True)
    
    class Meta:
        model = Order
        fields = ['order_id','order_client_id','order_address','order_date','order_product_units','order_active']
